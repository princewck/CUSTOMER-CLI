...
function printRows (rows) {
	...
	for (j = 0; j < numCols; j++) {
		...
		for (i = 0; i < rows.length; i++) {
			maxLengthForColumn = Math.max(getTextLength(getFormattedString(rows[i][j], !i || !j)), maxLengthForColumn);
		}
		...
		for (i = 0; i < rows.length; i++) {
			padding = maxLengthForColumn - getTextLength(getFormattedString(rows[i][j], !i || !j));
...
console._table = function(data, keys) {
